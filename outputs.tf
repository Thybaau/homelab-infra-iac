...
# === Outputs VM JAROD ===
output "jarod_vm" {
  description = "Informations de la VM JAROD"
  value = {
    name       = proxmox_vm_qemu.jarod.name
    vmid       = proxmox_vm_qemu.jarod.vmid
    ip_address = proxmox_vm_qemu.jarod.default_ipv4_address
    memory     = proxmox_vm_qemu.jarod.memory
    cores      = var.jarod_vm_cores
    disk_size  = var.jarod_vm_disk_size
  }
}
...
output "all_vms_summary" {
  ...
  value = {
    ...
    total_memory = var.k3s_master_memory + (var.k3s_worker_memory * (var.k3s_count - 1)) + var.jarod_vm_memory
    total_cores  = (var.k3s_vm_cores * var.k3s_count) + var.jarod_vm_cores
